	...
	---------------------
		...
		Begin Transaction
			...
			Begin Try
				...
				From
				(
					SELECT Row_Number() Over ( Order by ord.[CreateTime] Desc ) as [ROWID],							
							  ord.[OrderID],
							  ord.[OrderUserCode],
							  ord.[OrderPhone],
							  ord.[OrderStatus],
							  ...
							  ord.[CreateTime],
							  ord.[OrderFullname],
							  ord.[OrderShipAddress]
					FROM dbo.[ORDER] ord
					WHERE 
					(
						( @txtSearch IS NULL OR @txtSearch = '' OR ord.[OrderCode] LIKE '%' + @txtSearch + '%' )
